...
const sendMessage = async (userId, to, message) => {
    // 1. Check readiness
    if (status !== 'READY') throw new Error('WhatsApp client not ready. Scan QR first.');

    // 2. Format Number
    let chatId = to.includes('@c.us') ? to : `${to}@c.us`;
    if (!to.includes('@')) chatId = `${to.replace(/\D/g, '')}@c.us`;

    // 3. Real Send (if client active)
    if (client) {
        try {
            const msg = await client.sendMessage(chatId, message);
            return { sid: msg.id.id };
        } catch (e) {
            // If real send fails (e.g. disconnected), throw
            console.error('Real Send Failed:', e);
            throw e;
        }
    }

    // 4. Simulated Send (Fallback / Vercel Mode)
    else {
        console.log(`[SIMULATION] Sending to ${to}: ${message}`);
