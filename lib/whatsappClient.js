...
const initializeClient = async (userId) => {
    ...
    try {
        ...
        // on Vercel, real Puppeteer is too heavy/unstable for persistent connection.
        // We force SIMULATION MODE for the demo to ensure the UI works.
        const isVercel = process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_VERSION;

        if (isVercel) {
            log('Environment: Vercel - Forcing SIMULATION MODE for stability.');
            status = 'Simulation Mode';

            // Fake QR generation delay
            setTimeout(async () => {
                await prisma.user.update({
                    where: { id: userId },
                    data: {
                        waStatus: 'QR_READY',
                        // A static QR that points to a "This is a Demo" text or similar
                        waQrCode: 'https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=DEMO_MODE_ACTIVE'
                    }
                });
            }, 1000);

            // Fake "Ready" state after scanning (auto-scan simulation)
            // We can provide a button in UI to "Simulate Scan" or just auto-transition
            setTimeout(async () => {
                await prisma.user.update({
                    where: { id: userId },
        ...
        // ... Original Real Puppeteer Logic for Local/Docker ...
        log('Initializing WhatsApp Client (Real Browser)...');
